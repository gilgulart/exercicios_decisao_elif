...
class MaldicaoPessoal:
    ...
    def PreSegundaLuta(self, player):
        ...
        Depois de uma longa jornada, enfrentando todo tipo de criatura e atravessando os mais diversos biomas de Eldoria,
        {player.o} jovem finalmente alcança as regiões mais profundas do mundo.""")

        dialogosPreSegundaLuta.append(f"""
        Seu objetivo é claro: enfrentar o grandioso Gilberto, o Pombo, e conquistar o lendário Elixir da Vida — sua única
        ...
        dialogosPreSegundaLuta.append(f"""
        Caminhando pela região mais profunda, o ar se torna pesado, e o silêncio, inquietante. À sua frente, surge uma 
        caverna sombria. Apenas ao olhar, {player.nome} já tem certeza — aquele é o covil de Gilberto.""")
        
        dialogosPreSegundaLuta.append(f"""
        Sem hesitar, {player.ele} entra.""")
        
        dialogosPreSegundaLuta.append(f"""
        Lá dentro, uma cena inesperada: Gilberto está tranquilamente sentado, lendo um livro de filosofia, como se nada 
        ...
        dialogosPreSegundaLuta.append(f"""
        O que realmente importa está ali: o Elixir da Vida… a única coisa que {player.o} separa da morte.""")
        
        dialogosPreSegundaLuta.append(f"""
        O coração de {player.nome} dispara.""")
    ...
    def PosSegundaLuta(self, player):
        ...
        dialogosPosSegundaLuta.append(f"""
        O impacto ecoa pela caverna, levantando poeira e silenciando tudo ao redor. {player.oMaiusculo} jovem permanece
        de pé, quase sem forças, sentindo o peso de cada ferimento.""")

        dialogosPosSegundaLuta.append(f"""
        O Elixir da Vida está logo ali.""")

        dialogosPosSegundaLuta.append(f"""
        Mas antes que possa pegá-lo, uma voz fraca {player.o} interrompe.""")

        dialogosPosSegundaLuta.append(f"""
        — “Então… você venceu a morte?”""")

        dialogosPosSegundaLuta.append(f"""
        {player.oMaiusculo} jovem olha para Gilberto, ainda vivo, respirando com dificuldade.""")

        dialogosPosSegundaLuta.append(f"""
        — “Ainda não. Mas vou.”""")

        dialogosPosSegundaLuta.append(f"""
        Gilberto solta uma leve risada, cansada.""")

        dialogosPosSegundaLuta.append(f"""
        — “Curioso… você passou por tudo isso… só para adiar o inevitável.”""")

        dialogosPosSegundaLuta.append(f"""
        {player.oMaiusculo} {player.menino} franze a testa.""")

        dialogosPosSegundaLuta.append(f"""
        — “Não é sobre adiar. É sobre viver.”""")

        dialogosPosSegundaLuta.append(f"""
        — “Viver?” — Gilberto repete. — “Você chama isso de viver? Fugir da morte a qualquer custo?”""")

        dialogosPosSegundaLuta.append(f"""
        {player.oMaiusculo} jovem aperta os punhos.""")

        dialogosPosSegundaLuta.append(f"""
        — “Eu lutei porque não queria morrer sem propósito.”""")

        dialogosPosSegundaLuta.append(f"""
        Silêncio.""")

        dialogosPosSegundaLuta.append(f"""
        Gilberto observa, como se estivesse avaliando cada palavra.""")

        dialogosPosSegundaLuta.append(f"""
        — “E agora você tem um?”""")

        dialogosPosSegundaLuta.append(f"""
        {player.nome} hesita… mas responde:""")

        dialogosPosSegundaLuta.append(f"""
        — “Tenho. Eu escolhi lutar.”""")

        dialogosPosSegundaLuta.append(f"""
        Gilberto fecha os olhos por um instante.""")

        dialogosPosSegundaLuta.append(f"""
        — “Interessante… a maioria só percebe que quer viver quando está prestes a morrer.”""")

        dialogosPosSegundaLuta.append(f"""
        {player.oMaiusculo} jovem pega o elixir.""")

        dialogosPosSegundaLuta.append(f"""
        — “Então você fez isso de propósito?”""")

        dialogosPosSegundaLuta.append(f"""
        — “Não.” — Gilberto responde, com um leve sorriso. — “Mas o medo… costuma revelar quem realmente somos.”""")

        dialogosPosSegundaLuta.append(f"""
        {player.nome} encara o frasco por um momento.""")

        dialogosPosSegundaLuta.append(f"""
        — “E o que eu sou?”""")

        dialogosPosSegundaLuta.append(f"""
        Gilberto respira fundo, já no fim.""")

        dialogosPosSegundaLuta.append(f"""
        — “Alguém que finalmente entende que viver não é ter tempo infinito… é saber o que fazer com o tempo que tem.”""")

        dialogosPosSegundaLuta.append(f"""
        Silêncio.""")

        dialogosPosSegundaLuta.append(f"""
        {player.oMaiusculo} jovem bebe o elixir.""")

        dialogosPosSegundaLuta.append(f"""
        A maldição desaparece.""")

        dialogosPosSegundaLuta.append(f"""
        Quando {player.ele} olha novamente, Gilberto já não está mais vivo.""")

        dialogosPosSegundaLuta.append(f"""
        E, pela primeira vez… a vida não parece apenas algo a ser preservado — mas algo a ser vivido de verdade.""")
