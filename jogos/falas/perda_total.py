...
class PerdaTotal:
    ...
    def PosPrimeiraLuta(self, player):
        ...
        dialogosPosPrimeiraLuta.append(f"""A dor se transforma em determinação.""")
        
        dialogosPosPrimeiraLuta.append(f"""Se quiser vingar sua família, {player.ele} precisará se tornar mais forte.""")
        
        dialogosPosPrimeiraLuta.append(f"""E assim, com o mapa em mãos e um único objetivo em mente, {player.o} jovem inicia sua jornada — não por salvação… mas por vingança.""")
    ...
    def PreSegundaLuta(self, player):
        ...
        dialogosPreSegundaLuta.append(f"""A luta de sua vida.""")
        
        dialogosPreSegundaLuta.append(f"""Sem mais dúvidas. Sem mais medo.""")

        dialogosPreSegundaLuta.append(f"""Apenas ódio… e determinação.""")
    ...
    def PosSegundaLuta(self, player):
        ...
        dialogosPosSegundaLuta.append(f"""— “E então…” — diz Gilberto, com voz fraca — “isso vai te trazer paz?”""")
        
        dialogosPosSegundaLuta.append(f"""{player.o} jovem não hesita:""")
        
        dialogosPosSegundaLuta.append(f"""— “Vai.”""")
        
        dialogosPosSegundaLuta.append(f"""Gilberto observa em silêncio por um instante.""")
        ...
        dialogosPosSegundaLuta.append(f"""— “Eu sei.” — responde Gilberto, sem resistência.""")

        dialogosPosSegundaLuta.append(f"""— “Mas me diga… sua dor começou comigo?”""")

        dialogosPosSegundaLuta.append(f"""{player.o} jovem trava.""")

        dialogosPosSegundaLuta.append(f"""— “A dor sempre encontra um motivo.” — diz Gilberto.""")

        dialogosPosSegundaLuta.append(f"""— “Mas ela não termina quando o motivo desaparece.”""")

        dialogosPosSegundaLuta.append(f"""{player.nome} aperta a arma com mais força.""")                

        dialogosPosSegundaLuta.append(f"""— “Você é o motivo.”""")   

        dialogosPosSegundaLuta.append(f"""Gilberto balança levemente a cabeça.""")   

        dialogosPosSegundaLuta.append(f"""— “Não. Eu fui a causa… naquele momento. Mas o que você sente agora… já é seu.”""")   

        dialogosPosSegundaLuta.append(f"""Silêncio.""")   

        dialogosPosSegundaLuta.append(f"""A lâmina treme levemente.""")   

        dialogosPosSegundaLuta.append(f"""— “Se eu te matar… isso acaba.”""")   

        dialogosPosSegundaLuta.append(f"""— “Ou começa outra coisa.” — responde Gilberto.""")   

        dialogosPosSegundaLuta.append(f"""— “A vingança não encerra histórias… ela as continua.”""")

        dialogosPosSegundaLuta.append(f"""{player.o} jovem fica em silêncio.""")   

        dialogosPosSegundaLuta.append(f"""O ódio ainda está ali… mas algo mudou.""")   

        dialogosPosSegundaLuta.append(f"""— “Então o que você quer que eu faça?” — pergunta, quase em um sussurro.""")   

        dialogosPosSegundaLuta.append(f"""Gilberto solta um último suspiro.""")   

        dialogosPosSegundaLuta.append(f"""— “Nada. Pela primeira vez… escolha por você. Não por mim… e nem pela dor.”""")   

        dialogosPosSegundaLuta.append(f"""Silêncio absoluto.""")  

        dialogosPosSegundaLuta.append(f"""A arma ainda está levantada.""")  
